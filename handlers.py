...
1️⃣ ভিডিও আপলোড করুন (MP4, AVI, MKV)
2️⃣ ভিডিওর ভাষা সিলেক্ট করুন
3️⃣ আমার কাজ শেষ করার অপেক্ষা করুন
4️⃣ ডাবিংকৃত ভিডিও ডাউনলোড করুন
...
*ধাপ ৩:* অপেক্ষা করুন
- প্রক্রিয়াকরণে ৩-১০ মিনিট সময় লাগে
- /status দিয়ে অগ্রগতি দেখতে পারেন
...
/cancel - চলমান কাজ বাতিল করুন
    ...
    async def language_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for code, name in LANGUAGES.items():
            if code == "bn":
                continue
            btn = InlineKeyboardButton(name, callback_data=f"lang_{code}")
            row.append(btn)
            if len(row) == 2:
                keyboard.append(row)
                row = []
...
⏳ অনুগ্রহ করে অপেক্ষা করুন...
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if any(word in text for word in ["হ্যালো", "hello", "হাই", "hi"]):
            await update.message.reply_text(
                "👋 হ্যালো! ভিডিও আপলোড করুন বাংলা ডাবিংয়ের জন্য।"
            )
        elif any(word in text for word in ["ধন্যবাদ", "thank", "thanks"]):
            await update.message.reply_text(
    ...
    async def handle_video(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if duration > 3600:
            await update.message.reply_text(
                f"❌ সর্বোচ্চ ১ ঘণ্টার ভিডিও সাপোর্টেড!\n"
        ...
        if file_size > 500 * 1024 * 1024:
            await update.message.reply_text(
                f"❌ সর্বোচ্চ ৫০০ MB সাইজ সাপোর্টেড!\n"
        ...
        status_msg = await update.message.reply_text(
            ...
            f"🆔 আইডি: `{self.user_sessions[user_id]['job_id']}`",
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    async def process_video(
        ...
        try:
            ...
            output_path = f"outputs/{user_id}_{int(datetime.now().timestamp())}_bangla.mp4"
            ...
            async def update_progress(progress: float, stage: str):
                ...
                # Send updates every 10%
                if int(progress) % 10 == 0 and status_msg_id:
                    try:
                        await self.app.bot.edit_message_text(
                            f"🔄 *প্রক্রিয়াকরণ চলছে...*\n"
                            f"📊 অগ্রগতি: {int(progress)}%\n"
                            f"📌 ধাপ: {stage}\n"
